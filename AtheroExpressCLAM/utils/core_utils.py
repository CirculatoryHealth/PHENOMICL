...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, tot_acc, model, ckpt_name):
        ...
        if self.verbose:
            print(
                f"Weighted accuracy increased ({self.tot_acc_min:.6f} --> {tot_acc:.6f}).  Saving model ..."
            )
...
def train(datasets, cur, args):
    ...
    scheduler = ReduceLROnPlateau(
        optimizer=optimizer, mode="min", factor=0.1, patience=10, verbose=True
    )
    # Data loaders
    ...
    for i in range(args.n_classes):
        ...
        print(
            "class {}: acc {}, correct {}/{}".format(i, acc, correct, count), flush=True
        )
...
def train_loop_clam(
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                "batch {}, loss: {:.4f}, instance_loss: {:.4f}, weighted_loss: {:.4f}, Y_prob: {:.4f}, Y_true: {:.4f}".format(
                    batch_idx,
                    loss_value,
                    instance_loss_value,
                    total_loss.item(),
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                "batch {}, loss: {:.4f}, label: {}, bag_size: {}, Y_prob: {}".format(
                    batch_idx, loss_value, label.item(), data.size(0), Y_prob[0]
                ),
                flush=True,
            )
